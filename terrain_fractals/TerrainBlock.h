...
class TerrainBlock
{
	...
	float *pts;     //(n+1) * (n+1) points in 3D space [x,y,z]
	float *normals; //(n+1) * (n+1) normals in 3D space [x,y,z] 
	float *colors;  //(n+1) * (n+1) colors in 3D space [r,g,b]
	float *triangleNorms;
	int n;  //the dimension of the block
